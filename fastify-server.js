...
function getLastIdInStudent() {
    //get the last in students
    return null
}

function appendsToStudentObject(sd) {
    students = [...students, { first: sd.first, last: sd.last, id: getLastIdInStudent() }]
}
...
fastify.post("/cit/student", function(request, reply) {
    ...
    appendsToStudent(newStudent);
    reply
        .code(200) // status code
        .header("Content-Type", "application/json; charset=utf-8") // mime type for arrays and jsons
        .send(students); // we need to send back the updated student arry
});
...
fastify.get("/cit/student/:id", function(request, reply) {
    ...
    */
    reply
        ...
        .send(resp); // this line needs to change
});
